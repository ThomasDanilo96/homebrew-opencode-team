...
const DAILY_CLASS_LIMITS = Object.freeze({ quick: { minutes: 5, toolCalls: 8, delegations: 1 }, normal: { minutes: 15, toolCalls: 20, delegations: 1 }, complex: { minutes: 45, toolCalls: 50, delegations: 3 }, long: { minutes: 120, toolCalls: 100, delegations: 10 } });
...
export const beginRequestCycle = (state = createGuardrailState(), text, now = Date.now(), env = process.env) => {
  ...
  const classification = classifyRequest(text), limitsByClass = env.OPENAI_DAILY_PROFILE === "1" ? DAILY_CLASS_LIMITS : CLASS_LIMITS, base = limitsByClass[classification], override = requestOverrides(text);
  ...
  return { ...state, objective: String(text || "").trim(), limits, startedAt: now, toolCalls: 0, weightedUnits: 0, delegations: 0, activeDelegations: 0, activeDelegation: false, verificationTerminal: false, budgetTerminal: false, classification, checkpointed: classification === "long", stopped: resumed ? false : Boolean(state.stopped) || isStopText(text) };
};
...
export const createGuardrailState = (env = process.env, now = Date.now()) => ({ limits: readGuardrailLimits(env), startedAt: now, toolCalls: 0, weightedUnits: 0, delegations: 0, activeDelegations: 0, activeDelegation: false, stopped: false, verificationTerminal: false, budgetTerminal: false, classification: "normal", checkpointed: false, objective: null });
