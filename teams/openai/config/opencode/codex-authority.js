...
const PROTECTED_IDENTITY = new Set(["status", "version", "schema_version", "session_id", "agent", "task_fingerprint", "task_lease_id", "task_call_id", "packet_id", "master_parent_session_id", "attempt", "objective_sha256", "codex_run_id", "thread_id", "codex_thread_id", "parent_codex_run_id"]);
...
export const ensurePolicy = async (sessionID, metadata = {}) => { if (!root() || !sessionID) return null; return withLock(sessionID, async () => {
  const current = await readPolicy(sessionID);
  if (current) {
    const identity = [...PROTECTED_IDENTITY].filter((key) => !["status", "version", "schema_version", "session_id"].includes(key));
    if (identity.some((key) => current[key] != null && metadata[key] != null && current[key] !== metadata[key])) throw new PolicyConflictError({ task_id: sessionID, reason: "protected_identity_conflict" });
    const repaired = { ...current };
    let changed = false;
    for (const key of identity) if (repaired[key] == null && metadata[key] != null) { repaired[key] = metadata[key]; changed = true; }
    if (changed) await atomicWrite(sessionID, repaired);
    return repaired;
  }
  const policy = { ...callerMetadata(metadata), schema_version: 1, version: 1, session_id: sessionID, status: CODEX_REQUIRED, created_at: Date.now() }; await atomicWrite(sessionID, policy); return policy;
}); };
