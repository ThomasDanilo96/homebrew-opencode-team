...
const MUTATING_ACTION = /\b(?:add|create|modify|change|implement|refactor|migrat(?:e|ion)?|patch|delete|rename|update|deploy)\b|\bfix(?:es|ed|ing)?\s+(?!(?:already|existing|applied)\b)|\bwrite\s+(?:code|file|files|script|tests?|implementation|changes?)\b|\b(?:apply|make)\s+(?:the\s+)?(?:changes?|fix(?:es)?|patch(?:es)?)\b/i;
const NEGATED_MUTATION = /\b(?:do\s+not|don['’]t|must\s+not|never)\s+(?:(?:ever|also|actually|just)\s+)*(?:add|create|modify|change|implement|refactor|migrat(?:e|ion)?|patch|delete|rename|update|deploy|fix(?:es|ed|ing)?|write\s+(?:code|file|files|script|tests?|implementation|changes?)|(?:apply|make)\s+(?:the\s+)?(?:changes?|fix(?:es)?|patch(?:es)?))(?:\s+(?:or|and)\s+(?:add|create|modify|change|implement|refactor|migrat(?:e|ion)?|patch|delete|rename|update|deploy|fix(?:es|ed|ing)?|write\s+(?:code|file|files|script|tests?|implementation|changes?)|(?:apply|make)\s+(?:the\s+)?(?:changes?|fix(?:es)?|patch(?:es)?)))*\b/gi;
