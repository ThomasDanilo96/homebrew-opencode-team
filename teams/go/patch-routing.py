#!/usr/bin/env python3
"""
apply_background_routing_fix.py — Build candidate OMO from certified V2 baseline.
Applies: four no_attach sentinels + call_omo_agent foreground-only enforcement.
Does NOT apply: clickability metadata experiment.
"""
import os
import sys
...
if len(sys.argv) != 2:
    print("Usage: apply_background_routing_fix.py <file>", file=sys.stderr); sys.exit(1)
...
if v1_count != 0:
    print("REFUSED: old foreground V1 patch present", file=sys.stderr); sys.exit(1)

bg_v2_count = src.count(BG_V2_SENTINEL)
if bg_v2_count != 1:
    print(f"REFUSED: BG V2 sentinel count == {bg_v2_count} (expected 1)", file=sys.stderr); sys.exit(1)

nc = src.count(NATIVE_SENTINEL)
tc = src.count(TASK_SENTINEL)
bc = src.count(BG_TASK_SENTINEL)
cc = src.count(BG_CALL_OMO_SENTINEL)
fg = src.count(FG_ONLY_SENTINEL)
bt = src.count(BUILDER_TASK_SENTINEL)

all_native = [nc, tc, bc, cc]
if all(x == 0 for x in all_native):
    pass
...
if src.count(BG_V2_SENTINEL) != 1:
    print(f"ERROR: BG V2 sentinel count != 1", file=sys.stderr); sys.exit(1)
if OLD_V1_SENTINEL in src:
    print("ERROR: old V1 sentinel present", file=sys.stderr); sys.exit(1)
if src.count("_GO_CLICKABILITY_FIX_V1") != 0:
    print("ERROR: clickability fix sentinel present (should be removed)", file=sys.stderr); sys.exit(1)
